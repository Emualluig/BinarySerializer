...
template<typename T> void serialize(const std::unordered_set<T>& data, OutByteStream& obs) {
    serialize(data.size(), obs);
    for (const T& elem : data) {
        serialize(elem, obs);
    }
...
template<typename T, typename U> void serialize(const std::unordered_map<T, U>& data, OutByteStream& obs) {
    serialize(data.size(), obs);
    for (const auto& [key, value] : data) {
        serialize(key, obs);
        serialize(value, obs);
